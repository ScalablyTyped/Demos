...
/**
  * This object is dedicated to display the correct example chosen by the user.
  * When the user changes an example, we stop the current animation, we load the
  * next one and we fetch the Scala source code from Github, displaying it
...
object ExampleSelector {
  ...
  def changeCanvas(example: PIXIExample): html.Canvas = {
    ...
    for (idx <- 0 until children.length)
      canvasContainer.removeChild(children(idx))
  ...
  def main(args: Array[String]): Unit = {
    ...
    PIXIExample.allExamples.foreach {
      case (title, examples) => makeSection(title, examples)
    }
